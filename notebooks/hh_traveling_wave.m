...
function a = hh_traveling_wave(c, ah, l, N)

% Input  : - c,  speed (must be > 1)
%          - ah, ambient height
%          - l,  half-length of the domain
%          - N,  number of Fourier modes used in the computation
% Output : hh_traveling_wave_init.txt file with the solution (x, u(x))
% Example: hh_traveling_wave(1.2)
    ...
    L = ceff*(4 + 5 * k.^2 + k.^4) - 8*ah; % linear operator
    ...
    while (err > tol)
        iter = iter + 1;
        u0_hat = fft(u0);
        Nl_hat = fft(u0 .* ifft(k.^4 .* u0_hat) ...
                    + ifft(k .* u0_hat) .* ifft(k.^3 .* u0_hat) ...
                    - 0.5 * ifft(k.^2 .* u0_hat).^2 ...
                    + 5.0 * u0 .*  ifft(k.^2 .* u0_hat) ...
                    + 2.5 * ifft(k .* u0_hat).^2 ...
                    + 6 * u0.^2);
        Nl_hat(j) = 0;
        Lu0_hat = L .* u0_hat;
    ...
    io = fopen('hh_traveling_wave_init.txt','w');
    fprintf(io, '# x u0 \n');
